...
<link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300&display=swap" rel="stylesheet">
    ...
    if (isset($_POST["Name"])) {
      ...
      if (isset($_GET['red_id'])) {
          $sql = mysqli_query($link, "UPDATE `clients` SET `Name` = '{$_POST['Name']}', `Email` = '{$_POST['Email']}' , `Phone` = '{$_POST['Phone']}', `dt_added` = '{$_POST['dt_added']}' WHERE `ID`={$_GET['red_id']}");
      } else {
          
          $sql = mysqli_query($link, "INSERT INTO `clients` (`Name`, `Email`, `Phone`, `dt_added`) VALUES ('{$_POST['Name']}', '{$_POST['Email']}', '{$_POST['Phone']}', '{$_POST['dt_added']}')");
      }
    ...
    if (isset($_GET['red_id'])) {
      $sql = mysqli_query($link, "SELECT `ID`, `Name`, `Email`, `Phone`, `dt_added` FROM `clients` WHERE `ID`={$_GET['red_id']}");
